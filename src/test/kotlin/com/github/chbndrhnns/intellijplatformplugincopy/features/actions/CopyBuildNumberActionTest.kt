...
class CopyBuildNumberActionTest : TestBase() {
    ...
    fun testActionIsRegistered() {
        ...
        val action =
            actionManager.getAction("com.github.chbndrhnns.intellijplatformplugincopy.features.actions.CopyBuildNumberAction")
        assertNotNull("Action should be registered in plugin.xml", action)
        assertTrue(
            "Registered action should be CopyBuildNumberAction",
